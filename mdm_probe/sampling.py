...
from .metrics import information_gain, max_p_gain, p_gt_gain, remaining_positions, score_p_gt, score_values
...
def run_single_anchor_probe(
    ...
    *,
    ...
    selection_metric: str = "information_gain",
) -> list[dict]:
    ...
    for pos, after in zip(candidates, anchored):
        ...
        max_gain_sum, max_gain_by_target = max_p_gain(baseline, after, exclude=[pos])
        rows.append(
            {
                ...
                "max_p_gain": max_gain_sum,
                "max_p_gain_by_target": max_gain_by_target,
                ...
                "max_p_score_before": score_values(baseline.max_p, [pos], reduction=score_reduction),
                "max_p_score_after": score_values(after.max_p, [pos], reduction=score_reduction),
                "p_gt_before": list(baseline.p_gt),
                "p_gt_after": list(after.p_gt),
                "max_p_before": list(baseline.max_p),
                "max_p_after": list(after.max_p),
...
def run_greedy_k_probe(
    ...
    *,
    ...
    metric_index = {"information_gain": 0, "p_gt_gain": 4, "max_p_gain": 6}[selection_metric]
    ...
    for step in range(1, min(max_k, len(eligible)) + 1):
        ...
        scored: list[tuple[float, int, ConfidenceResult, list[float], float, list[float], float, list[float]]] = []
        for candidate, after in zip(candidates, candidate_results):
            ...
            max_gain_sum, max_gain_by_target = max_p_gain(current, after, exclude=exclude)
            scored.append((ig_sum, candidate, after, ig_by_target, p_gain_sum, p_gain_by_target, max_gain_sum, max_gain_by_target))
        ...
        best_ig, best_pos, best_result, best_ig_by_target, best_p_gain, best_p_by_target, best_max_gain, best_max_by_target = max(
            scored,
            key=lambda item: item[metric_index],
        )
        best_scores = {
            "information_gain": best_ig,
            "p_gt_gain": best_p_gain,
            "max_p_gain": best_max_gain,
        }
        previous = current
        ...
        step_rows.append(
            {
                ...
                "selection_metric": selection_metric,
                "selection_score": best_scores[selection_metric],
                "information_gain": best_ig,
                "information_gain_by_target": best_ig_by_target,
                "p_gt_gain": best_p_gain,
                "p_gt_gain_by_target": best_p_by_target,
                "max_p_gain": best_max_gain,
                "max_p_gain_by_target": best_max_by_target,
                "score_after": score_p_gt(current.p_gt, anchors, reduction=score_reduction),
                "max_p_score_after": score_values(current.max_p, anchors, reduction=score_reduction),
                "p_gt_before": list(previous.p_gt),
                "p_gt_after": list(current.p_gt),
                "max_p_before": list(previous.max_p),
                "max_p_after": list(current.max_p),
...
def run_layout_control_probe(
    ...
    *,
    ...
    for k in range(1, min(max_k, len(eligible)) + 1):
        for name, anchors in layout_anchors(eligible, k, greedy_chain, greedy_name).items():
    ...
    for (k, name, anchors), after in zip(specs, results):
        ...
        max_gain_sum, max_gain_by_target = max_p_gain(baseline, after, exclude=anchors)
        rows.append(
            {
                "example_id": encoded.example_id,
                "k": k,
                ...
                "max_p_score_before": score_values(baseline.max_p, anchors, reduction=score_reduction),
                "max_p_score_after": score_values(after.max_p, anchors, reduction=score_reduction),
                "p_gt_default": list(baseline.p_gt),
                "p_gt_before": list(baseline.p_gt),
                "p_gt_after": list(after.p_gt),
                "max_p_default": list(baseline.max_p),
                "max_p_before": list(baseline.max_p),
                "max_p_after": list(after.max_p),
                "entropy_before": list(baseline.entropy),
                "entropy_after": list(after.entropy),
