...
This is the archived chronological README from the exploratory phase. For the
selected step-6,000 anchor-lookahead method and current commands, use the
...
```bash
LIMIT=500 BATCH=8 bash Token2Token/experiments/scripts/run_adaptive_validation.sh
```
...
```bash
python3 -m Token2Token.main.precompute_anchor_targets \
  --examples 7473 \
  --anchors 5 \
  --ig-batch-size 64 \
  --resume \
...
```bash
bash Token2Token/experiments/scripts/run_anchor_order.sh
```
...
```bash
python3 -m Token2Token.experiments.precompute_rollout_targets \
  --examples 7473 \
  --anchors 2 \
  --rollout-k 2 \
  --rollout-steps 4 \
...
```bash
python3 -m Token2Token.experiments.precompute_local_unlock_targets \
  --examples 7473 \
  --anchors 2 \
  --window-size 9 \
...
```bash
python3 -m Token2Token.main.precompute_threshold_unlock_targets \
  --examples 5 \
...
bash Token2Token/experiments/scripts/run_threshold_unlock.sh
```
...
```bash
bash Token2Token/experiments/scripts/run_threshold_eval_compare.sh
```
...
```bash
bash Token2Token/experiments/scripts/run_anchor_transition_v2.sh
bash Token2Token/experiments/scripts/run_anchor_transition_v2_eval50.sh
```
...
```bash
bash Token2Token/experiments/scripts/run_anchor_transition_v3_kl.sh
```
...
```bash
MAX_STEPS=5 DATASET=gsm8k bash Token2Token/experiments/scripts/run_train.sh
```
...
```bash
MAX_STEPS=5 DATASET=lm1b bash Token2Token/experiments/scripts/run_train.sh
```
...
```bash
python3 -m Token2Token.experiments.decode \
...
```bash
python3 -m Token2Token.experiments.train_standard \
  --max-steps 7473 \
...
```bash
bash Token2Token/experiments/scripts/run_gsm8k_k_sweep.sh
```
