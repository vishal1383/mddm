...
threshold_tag="$(python3 -c "from Token2Token.main.eval_threshold_gsm8k import threshold_tag; print(threshold_tag(float('$THRESHOLD')))" )"
...
if [[ ! -f "$BASE_PREDICTIONS" ]]; then
  run_bounded python3 -m Token2Token.main.eval_threshold_gsm8k \
...
if [[ -z "$adapter_path" ]]; then
  ...
  if [[ ! -f "$adapter_path/adapter_config.json" ]]; then
    ...
    run_bounded python3 -m Token2Token.experiments.train_all_states_confidence \
...
python3 -m Token2Token.experiments.micro_trial_gate "${gate_args[@]}" \
