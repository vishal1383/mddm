...
run_v5() {
  ...
  step "$name" env NAME="$name" THRESHOLD="$threshold" MAX_STEPS=1200 \
    ...
    bash Token2Token/experiments/scripts/run_threshold_matched_v5.sh
}
...
step "round 4 remainder" bash Token2Token/experiments/scripts/run_decoder_sweep50_round4.sh

# The full benchmark of the decoder result, which never ran.
step "full benchmark" env BATCH=8 LIMIT=1319 bash Token2Token/experiments/scripts/run_full_benchmark.sh
